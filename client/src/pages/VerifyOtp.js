...
function VerifyOtp() {
  ...
  const handleVerify = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await axios.post("https://user-authentication-system-2wl2.onrender.com/api/auth/verify-otp", {
        email,
        otp
      });
    ...
    <div className="min-h-screen flex items-center justify-center p-6 bg-transparent relative overflow-hidden">
      ...
      >
        <motion.div variants={itemVariants} className="text-center mb-10">
          ...
          <p className="text-white/70 font-light">Confirm your identity via OTP</p>
        </motion.div>

        <form onSubmit={handleVerify} className="space-y-6">
          <motion.div variants={itemVariants} className="space-y-1">
            <label className="text-[10px] uppercase tracking-[0.2em] font-bold text-white/50 px-1">Verification Email</label>
          ...
          <motion.div variants={itemVariants} className="space-y-1">
            <label className="text-[10px] uppercase tracking-[0.2em] font-bold text-white/50 px-1">6-Digit Code</label>
            <div className="relative group">
